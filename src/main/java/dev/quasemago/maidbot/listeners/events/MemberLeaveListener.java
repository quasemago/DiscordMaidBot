...
@Component
public class MemberLeaveListener implements GenericEventListener<MemberLeaveEvent> {
    @Autowired
    ...
    public Mono<Void> handle(MemberLeaveEvent event) {
        return Mono.just(event)
                .doOnSuccess(e -> e.getMember()
                        .ifPresent(member -> {
                            if (!member.isBot()) {
                                Logger.log.debug("Leave guild " + member.getUsername());
                                ...
                                if (serverGuild != null && serverGuild.getLogChannelId() != null && serverGuild.getLogFlags() != null) {
                                    ...
                                    if (logTypesSet.contains(LogTypes.MEMBER_LEAVE)) {
                                        e.getClient()
                ...
                .onErrorResume(er -> {
                    Logger.log.error("Error on MemberLeaveEvent: " + er.getMessage());
                    return Mono.empty();
                })
                .then();
    }
