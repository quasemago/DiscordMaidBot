...
public class MemberVoiceStateUpdateListener implements GenericEventListener<VoiceStateUpdateEvent> {
    ...
    public Mono<Void> handle(VoiceStateUpdateEvent event) {
        return Mono.just(event)
                ...
                .doOnSuccess(e -> {
                    ...
                    final GuildServer serverGuild = this.serversService.getGuildServerByGuildId(e.getCurrent().getGuildId().asLong());
    ...
    private void onMemberJoinVoiceChannel(final VoiceStateUpdateEvent event, final GuildServer serverGuild, final Member member) {
    ...
    private void onMemberLeaveVoiceChannel(final VoiceStateUpdateEvent event, final GuildServer serverGuild, final Member member) {
        final LogTypesSet logTypesSet = LogTypesSet.of(serverGuild.getLogFlags());
    ...
    private void onMemberMoveVoiceChannel(final VoiceStateUpdateEvent event, final GuildServer serverGuild, final Member member) {
        final LogTypesSet logTypesSet = LogTypesSet.of(serverGuild.getLogFlags());
        if (logTypesSet.contains(LogTypes.MEMBER_JOIN_VOICE)) {
            final Channel newChannel = event.getCurrent()
                    .getChannel()
